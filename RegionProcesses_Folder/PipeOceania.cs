...
*/
    ...
    static void Main(string[] args)
    {
        ...
        Oceania processOC = new Oceania();
        processOC.SetLvlMortality(Double.Parse(args[0]));
        processOC.SetLvlTransmissionRate(Double.Parse(args[1]));
        Console.WriteLine("Oceania mortalityrate: {0}", processOC.getLevelMortality());
        Console.WriteLine("Oceania spreadrate: {0}", processOC.getLevelSpread());        
        while (true){
            try{
                using (var pipeWriter = new NamedPipeClientStream(".", pipeName, PipeDirection.Out))
                {
                    ...
                    processOC.simulation();
                    var actualData = DateTime.Now.ToString() + "," + processOC.getTotalPopulation() + "," + processOC.getInfected() + "," + processOC.getUninfected() + "," + processOC.getDead() + "," + filename_id;
