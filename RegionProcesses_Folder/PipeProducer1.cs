...
class Producer
{
    ...
    public int _totalPopulation = 100000000;
    ...
    public bool _upgMortalityLvl = false;
    public int _pneededMort = 1;//cantidad de puntos que necesita para upgrade mortality lvl
    public double _lvlspread = 0.10; //1.00 es el max se va subiendo de 0.01 en 0.01
    public bool _upgSpreadLvl = false;
    ...
    */
    ...
    public double getLevelSpread() { return this._lvlspread; }
    ...
    public void simulation()
    {
        ...
        if (this._infected > 0)
        {//aqui se van agregando los infectados
            Console.WriteLine("inside if infected #1");
        ...
        if (this._infected > 0)
        {//aqui se van agregando los muertos
            Console.WriteLine("inside if infected #2");
            int newlyDead = (int)((this._infected / 2) * _lvlmortality);
            Console.WriteLine("newlydead: ", newlyDead);
        ...
        while (true)
        {
            try
            {
                using (var pipeWriter = new NamedPipeClientStream(".", pipeName, PipeDirection.Out))
                {
                    ...
                    producer.simulation();
                    var actualData = data + "," + producer.getTotalPopulation() + "," + producer.getInfected() + "," + producer.getUninfected() + "," + producer.getDead();
                    Console.WriteLine("actual--------------->: {0}", actualData);
